...
std::map<std::string, uint8_t> g_outputWorkerList;
...
static void configMwNfdConfig(const std::string configFileName)
{
	...
	for(const auto& section : mw_nfd_section) {
		...
		}else if( section.first == "output-thread-core-assign"){
			auto outputs = config.get_child("mw-nfd.output-thread-core-assign");
			for (const auto& output : outputs) {
				auto ret = g_outputWorkerList.emplace( output.first, output.second.get_value<std::uint8_t>() );
				if(ret.second == false){
					std::cerr << "There are Same Physical Port :" << output.first << std::endl;
					exit(0);
				}
			}
...
int main(int argc, char** argv)
{
  ...
  using namespace nfd;
	...
	for(auto & x:g_outputWorkerList){
		...
		int ifIndex= if_nametoindex(x.first.c_str());
		coreId = x.second;

		tg.create_thread( [coreId, ifIndex]{
				...
				if (rc != 0) {
                std::cerr << "The Output Thread : Error calling pthread_setaffinity_np: " << rc << "\n";
                    exit(0);
				}

				NFD_LOG_INFO("The Output-Thread is Running with core#: "<< coreId << "/WorkerId(ifIndex)#:" << ifIndex << " TID:" << sched_getcpu()); 
