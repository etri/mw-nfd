    ...
    if (wantListen && !v4Channel->isListening()) {
      ...
      if(getBulkFibTest()){
          ...
          if(faceUri0.getScheme()=="udp4") {
            std::cout << "+++++++++++++++++++++::" << faceUri0 << std::endl;
            udp::Endpoint localEndpoint;
            boost::system::error_code ec;

            boost::asio::ip::address ip_address0 =
                    boost::asio::ip::address::from_string(faceUri0.getHost(), ec);

            udp::Endpoint remoteEndpoint0(ip_address0,6363);

#ifdef ETRI_NFD_ORG_ARCH
            ip::udp::socket socket(getGlobalIoService(), remoteEndpoint0.protocol());
#else
            int ifIndex=getIfIndex(remoteEndpoint0.address().to_string().c_str());
            ip::udp::socket socket( *getGlobalIoService(ifIndex), remoteEndpoint0.protocol());
#endif

            time::nanoseconds m_idleFaceTimeout(1); 

            socket.set_option(ip::udp::socket::reuse_address(true));
            //socket.bind(localEndpoint);
            socket.connect(remoteEndpoint0);

            GenericLinkService::Options options;
            options.allowFragmentation = true;
            options.allowReassembly = true;
            options.reliabilityOptions.isEnabled = false;

            auto linkService = make_unique<GenericLinkService>(options);
            auto transport = make_unique<UnicastUdpTransport>(std::move(socket),  ndn::nfd::FACE_PERSISTENCY_PERSISTENT,
                    //time::seconds(1));
                    3_s);
            ...
            boost::asio::ip::address ip_address1 =
                    boost::asio::ip::address::from_string(faceUri1.getHost(), ec);

            udp::Endpoint remoteEndpoint1(ip_address1,6363);

#ifdef ETRI_NFD_ORG_ARCH
            ip::udp::socket socket1(getGlobalIoService(), remoteEndpoint1.protocol());
#else
            ifIndex=getIfIndex(remoteEndpoint1.address().to_string().c_str());
            ip::udp::socket socket1( *getGlobalIoService(ifIndex), remoteEndpoint1.protocol());
#endif

            socket1.connect(remoteEndpoint1);

            linkService = make_unique<GenericLinkService>(options);
            transport = make_unique<UnicastUdpTransport>(std::move(socket1),  ndn::nfd::FACE_PERSISTENCY_PERSISTENT,
                    3_s);
