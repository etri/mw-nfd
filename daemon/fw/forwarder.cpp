                              ...
                              const shared_ptr<pit::Entry>& pitEntry, const Interest& interest)
{
  ...
  if (nextHopTag != nullptr) {
    ...
    nextHopFace = m_faceTable.get(*nextHopTag);
...
Forwarder::onIncomingData(const FaceEndpoint& ingress, const Data& data)
{
...
#ifndef ETRI_DUAL_CS  // NFD original not use dual cs
  ...
  // CS insert
	...
	if (pitMatches.size() == 1) {
		...
		NFD_LOG_DEBUG("onIncomingData prefix matching =" << pitEntry->getName());
...
Forwarder::setExpiryTimer(const shared_ptr<pit::Entry>& pitEntry, time::milliseconds duration)
{
    ...
    pitEntry->expiryTimer = getScheduler().schedule(duration, 
        [=] { onInterestFinalize(pitEntry); });
}
